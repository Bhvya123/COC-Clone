...
###########################################################################################
            if self.level >= 3:
                ...
                for troop in troops:
                    if  self.position[0] - 2 <= troop.position[0] and troop.position[0] <= self.position[0] + 2 and self.position[1] - 2 <= troop.position[1] and troop.position[1] <= self.position[1] + 2:
                        troop.health -= 200
                        if troop.health <= 0:
                            troop.health = 0
                            troop.kill()  
                if self.position[0] - 2 <= King.position[0] and King.position[0] <= self.position[0] + 2 and self.position[1] - 2 <= King.position[1] and King.position[1] <= self.position[1] + 2:
                    King.health -= 200
                    if King.health <= 0:
                        King.health = 0
                        King.kill()
    ...
    def scan_for_targets(self, King):
        ...
        for troop in troops:
            if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.attack_radius**2:
                ...
                return
        for stealth in stealthArchers:
            if stealth.isInvisible():
                continue
            if (stealth.position[0] - self.position[0])**2 + (stealth.position[1] - self.position[1])**2 <= self.attack_radius**2:
                self.isShooting = True
                self.attack_target(stealth)
                return 
