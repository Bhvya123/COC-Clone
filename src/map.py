...
def printMap(V):
    ...
    for sta in stealthArchers:
        a= 2*sta.position[0]
        b = 2*sta.position[1]
        map_matrix[a][b] = archColor(sta) + Fore.BLACK + 'st'
        map_matrix[a+1][b] = archColor(sta) + Fore.BLACK + 'AR'
        map_matrix[a][b+1] = archColor(sta) + Fore.BLACK + 'CH'
        map_matrix[a+1][b+1] = archColor(sta) + Fore.BLACK + 'ER'

    for h in healers:
        a= 2*h.position[0]
        b = 2*h.position[1]
        map_matrix[a][b] = healerColor(h) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = healerColor(h) + Fore.BLACK + ' '
        map_matrix[a][b+1] = healerColor(h) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = healerColor(h) + Fore.BLACK + 'ER'
